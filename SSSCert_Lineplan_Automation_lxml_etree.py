# -*- coding: utf-8 -*-
"""
Created on Tue Oct  1 04:50:10 2024
...
#from lxml, import etree to parse through kml doc
...
def browse_kml():
    ...
    file_path = filedialog.askopenfilename(title="Select KML File", 
    ...
    if file_path:
        try:
            ...
            #define namespace, or style of kml so that this code can properly parse through. Ran into issue where code unable to read through kml without this
            namespace = {'kml': 'http://earth.google.com/kml/2.0'}
            
            #create list to store all coordinates
            all_coordinates = []
            
            #search through folders for ones with coordinates
            for folder in root.findall('.//kml:Folder', namespaces=namespace):
                placemarks = folder.findall('.//kml:Placemark', namespaces=namespace)
...
def spacing():
    ...
    #unique error for if number is entered, but it is not in scope
    elif number not in (50, 75, 100):
        messagebox.showerror("Error", "Now that number weren't right")
        return
    ...
    #set desired lenght of lines - ask as string to accept decimal points
    ...
    #set desired rotation (as string so even no value given returns 0)
    ...
    #prompt what file to be saved as
    new_filename = filedialog.asksaveasfilename(defaultextension=".lnw",
                                                title="Save File as", 
                                                filetypes=[("Line files", "*.lnw"),("All files", "*.*")])
    ...
    endlength = f"{len(all_coordinates)*12}"

    #initialize variable to append coordinates to from UTM conversion loop further down
    all_UTMCoords = []
    ...
    for x in all_coordinates:
        ...
        #use target position as center from which to draw out new endpoints for created lines
    ...
    #-----------------------------------------------------------------------------------------------------------
        ...
        for x in ToUTMList:
            ...
            #Grab just the easting/northing from the UTM & append values as this loop...loops
